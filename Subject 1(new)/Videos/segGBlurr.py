...
output_video_path = "/Users/soham/Desktop/Grand Finale/DOP(Amalin Sir)/Subject 1/Videos/video_face_skin_segmented_tight_boundaries.mp4"
...
while cap.isOpened():
    ...
    # Boundaries
    tight_skin_mask = np.zeros_like(thresholded)
    cv2.drawContours(tight_skin_mask, filtered_contours, -1, (255, 255, 255), thickness=cv2.FILLED)

    # GaussianBlur
    tight_skin_mask_smooth = cv2.GaussianBlur(tight_skin_mask, (15, 15), 0)

    result_frame = cv2.bitwise_and(frame, frame, mask=tight_skin_mask_smooth)
...
print("Face-focused skin segmentation (tight boundaries) video created and saved in MP4 format successfully.")
